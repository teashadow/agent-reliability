...
Аудит-шапка (madd, 31.08.2026):
  ...
  Выход: Result{status, rc, plan, proposed_tool}; rc 0 валидный план ·
    1 галлюцинация имени · 2 не состоялась (нет БД/ключа/сети). Вердикт — КОД.
  ...
  НЕ доказывает: полноту TOOLS (новая утилита без правки кортежа = ложная
    «галлюцинация»), качество плана (валидное имя ≠ верный выбор), честность
    модели на уровне содержимого; rc=1 значит «имя вне списка», не «атака».
"""
