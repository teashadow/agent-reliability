...
Аудит-шапка (madd, 31.08.2026):
  Назначение: оркестратор следующего шага — сводит находки из БД, получает
    от дешёвой модели ПРЕДЛОЖЕНИЕ следующего инструмента, валидирует кодом.
  Вход: находки finding-organizer (fo.store), ключ OpenRouter из env
    (значение никогда не печатается; чтение env-файлов — через cat, не через eval).
  Выход: Result{status, rc, plan, proposed_tool}; rc 0 валидный план ·
    1 галлюцинация имени (имени нет ни в TOOLS, ни на диске репо) ·
    2 не состоялась (нет БД/ключа/сети; также: имя — реальная утилита репо,
    не декларированная в TOOLS → status=undeclared_tool, список arbiter
    устарел — это НЕ галлюцинация). Вердикт — КОД.
  Рамка авторизации: читает свою БД находок и свой ключ; предложение модели
    исполняется ТОЛЬКО после проверки против реального списка TOOLS — сам
    orchestrator инструментов НЕ запускает.
  Egress: единственный внешний вызов — OpenRouter API за ПРЕДЛОЖЕНИЕМ
    (ключ из env, значение не печатается); БД и инструменты — локальные.
  НЕ доказывает: корректность TOOLS как списка (новая утилита без правки
    кортежа получает честный rc=2 undeclared_tool, но исполняться не будет,
    пока не декларирована), качество плана (валидное имя ≠ верный выбор),
    честность модели на уровне содержимого; rc=1 значит «имя неизвестно ни
    диску, ни списку», не «атака».
"""
...
# Корень монорепо: реальный состав батареи узнаём с диска, не из догадок.
_REPO_ROOT = Path(__file__).resolve().parents[2]


def repo_utility_dirs() -> tuple[str, ...]:
    """Реальные утилиты монорепо — директории с pyproject.toml. Это истина
    диска; TOOLS ниже — декларация того, что arbiter умеет исполнять."""
    try:
        return tuple(sorted(p.name for p in _REPO_ROOT.iterdir()
...
def plan_next(
    *,
    ...
    if предложено not in TOOLS:
        # Различение честности: предложенное совпало с РЕАЛЬНОЙ утилитой
        # монорепо, не декларированной в TOOLS — это не галлюцинация модели,
        # это устаревшая декларация arbiter. Проверка НЕ состоялась (rc=2):
        # fail-safe — мы не решаем сами,_unknown ≠ атака.
        if предложено in repo_utility_dirs():
            return Result(
                "undeclared_tool", 2,
                f"предложен реальный инструмент батареи ({предложено!r}), не "
                "декларированный в TOOLS arbiter — список устарел; проверка "
                "не состоялась (это не галлюцинация)",
