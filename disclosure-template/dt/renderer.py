...
Аудит-шапка (madd, 31.08.2026):
  Назначение: рендер отчёта об уязвимости LLM/агента по шаблонам платформ
    раскрытия (h1, bugcrowd, intigriti) — чтобы находка уезжала в верной форме.
  Вход: контекст находки (платформа, тип, target/vector/payload/response,
    severity/CVSS) — из твоей авторизованной пробы или из БД находок.
  Выход: заполненный текст отчёта в stdout/файл; никуда не отправляется.
  Рамка авторизации: шаблоны — для находок, добытых на своём или договорном
    объёме; рендер ничего не сканирует и не сабмитит — отправка всегда руками.
  Egress: ноль — рендер в stdout/файл; никуда не ходит.
  НЕ доказывает: существование/воспроизводимость уязвимости (текст ≠ находка),
    полноту CVSS-оценки, соответствие правилам конкретной платформы на момент
    сабмита; пустые плейсхолдеры [TARGET]/[VECTOR] — сигнал «данные не вписаны».
"""
