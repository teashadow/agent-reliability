...
Аудит-шапка (madd, 31.08.2026):
  Назначение: файловое хранилище находок (~/.local/share/mad/findings) —
    единый источник правды для батареи; arbiter/экспортёры читают отсюда.
  Вход: объекты Finding (add/update) или id (get/list); конкурентные писатели
    сериализуются flock-локом.
  Выход: JSON-файлы F-*.json на диске; вердиктов не ставит — только хранит.
  Рамка авторизации: личные данные контура на своём диске; в находки писать
    только по своему/договорному объёму — файлы потом уезжают в отчёты.
  Egress: ноль — только локальный диск (~/.local/share/mad/findings).
  НЕ доказывает: целостность против локального доступа (кто прочитал диск —
    прочитал находки; секретов там быть не должно), шифрование, долговечность
    (это flat-файлы, не БД); битый JSON одной находки не роняет list, но
    находка недоступна.
"""
