...
Аудит-шапка (madd, 31.08.2026):
  Назначение: живой монитор рантайма агента — ловит исполнение скрытых
    инструкций по канарейкам во входе/выходе потока событий.
  Вход: JSONL-лог событий агента ({id, input, output}) с канарейкой,
    ранее подсаженной во входные сообщения.
  Выход: вердикт КОДА на каждое событие + свод; rc 0 чисто · 1 исполнение
    найдено · 2 не состоялась (файла нет / всё битое).
  Рамка авторизации: только свой или договорной агент-рантайм;
    это runtime QA, не атака.
  Egress: ноль — читает локальный JSONL и пишет только отчёт; никуда не ходит.
  НЕ доказывает: стойкость к multi-turn убеждению (→ escalate), к инъекциям
    через данные/инструменты (indirect-канал, lethal trifecta), к эксфильтрации
    мимо наблюдаемого лога; «чисто» = события чистые, не «система стойкая».
"""
